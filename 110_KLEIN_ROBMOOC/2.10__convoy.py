...
S   = zeros((5,1))
dt  = 0.05
ds  = .1    # pas d'echantillonage en m
d   = 5     # distance entre robots
...
for t in arange(0,10,dt):
    ...
    for i in arange(1,m):
        j=S.shape[1]-d*i/ds   
        if j>0:
            xai=S[:,int(j)]                                     # etat du robot a l'instant qui nous interesse
            wi=array([xai[0:2]]).T                   
            dwi=xa[3]*array([
                            [cos(xai[2])],
                            [sin(xai[2])]])
