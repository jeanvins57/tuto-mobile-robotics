...
def f(x,w):
    x=x.flatten()
    φ,θ,ψ=x[3:6]                            # angles d'Euler
    vr=(x[6:9]).reshape(3,1)                # vitesse dans le repere du robot (on aurait pu considerer la vitesse dans le repere absolu)
    wr=(x[9:12]).reshape(3,1)               # vecteur de rotation represente dans le repere du robot
    w2=w*abs(w)                             # 
    τ=B@w2.flatten()                        #
    
    E=eulermat(φ,θ,ψ)                       # matrice d'euler
    dp=E@vr
...
def control(X):	
    ...
    #inverse of block 3
    wrd = 5*inv(eulerderivative(φ,θ,ψ))@array([[sawtooth(φd-φ)], [sawtooth(θd-θ)], [sawtooth(ψd-ψ)]])
    #inverso of block 2
